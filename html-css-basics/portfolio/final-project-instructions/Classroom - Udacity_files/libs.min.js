...
angular.module("ui.autocomplete",[]).directive("uiAutocomplete",["$timeout","$exceptionHandler",function(t,u){function m(d,b){var c=RegExp($.ui.autocomplete.escapeRegex(b),"i");return $.grep(d,function(b){return c.test($("<div>").html(b.label||b.value||b).text())})}var j=$.ui.autocomplete.prototype,k=j._initSource;$.extend(j,{_initSource:function(){this.options.html&&$.isArray(this.options.source)?this.source=function(d,b){b(m(this.options.source,d.term))}:k.call(this)},_normalize:function(d){return $.map(d,
function(b){return b&&"object"===typeof b?$.extend({label:b.label||b.value,value:b.value||b.label},b):{label:b+"",value:b}})},_renderItem:function(d,b){var c=b.groupLabel||b.label;if(b.groupLabel)c=$("<div>").append(c).addClass("ui-menu-group");else if(this.options.html){if("object"===typeof c&&(c=$(c)),"object"!==typeof c||1<c.length||!c.is("a"))c=$("<a>").append(c)}else c=$("<a>").text(c);return $("<li>").append(c).appendTo(d)},_resizeMenu:function(){var d=this;setTimeout(function(){var b=d.menu.element,
c=b.css("max-height")||0,f=Math.max(b.width("").outerWidth()+1,d.element.outerWidth());d.element.height();var i=$(window).height()-d.options.outHeight-b.offset().top;b.css({width:f,maxHeight:c&&i>c?c:i})},10)}});return{require:"ngModel",link:function(d,b,c,f){function i(a){angular.isObject(g)&&(!f.$viewValue&&0!==f.$viewValue?n(g):a&&a.item&&(a.item.label=angular.isObject(a.item.label)?$("<div>").append(a.item.label).html():a.item.label,angular.extend(g,a.item)),angular.forEach(f.$viewChangeListeners,
function(a){try{a()}catch(b){u(b)}}))}function j(){f.$setViewValue("");f.$render();i()}function k(){e.options.focusOpen&&!l&&b.autocomplete("search","")}function p(a){a=angular.isObject(a)?a:{};a.disabled=a.source?a.disabled:!0;a.minLength=a.focusOpen?0:a.minLength;a.outHeight=a.outHeight||0;a.position=a.position||{my:"left top",at:"left bottom",collision:"flipfit"};return a}function n(a){if(angular.isObject(a)){var b=/^\$/;angular.forEach(a,function(c,d){var e=typeof c;b.test(d)||("number"===e?a[d]=
0:"string"===e?a[d]="":"boolean"===e?a[d]=!1:angular.isObject(c)&&n(c))})}}var l=!1,h=null,q={},g=null,e=d.$eval(c.uiAutocomplete),r=b.val.bind(b),v="close destroy disable enable option search widget".split(" "),w="change close create focus open response search select".split(" "),x=d.$watch(c.ngModel,function(a){g=a;angular.isObject(g)&&(f.$formatters.push(function(a){return a.value}),f.$parsers.push(function(a){g.value=a;return g}),d.$watch(c.ngModel+".value",function(a){r()!==a&&(f.$viewValue=a,
f.$render())}),f.$setViewValue(g.value));a&&x()}),s={open:function(){l=!0;h=null},close:function(){l=!1},select:function(a,c){h=c;t(function(){b.blur()},0)},change:function(){var a=r(),a=!h||!h.item?e.options.onlySelect?"":a:h.item.value;f.$viewValue!==a&&d.$apply(function(){f.$setViewValue(a);f.$render();i(h)})}};angular.isObject(e)&&(e.methods={},e.options=p(e.options),angular.forEach(w,function(a){q[a]=function(b,c){if(s[a])s[a](b,c);if(e.events&&"function"===typeof e.events[a])e.events[a](b,c)}}),
e.options.appendTo||(e.options.appendTo=b.parents(".ng-view")[0]||b.parents("[ng-view]")[0]||null),angular.forEach(v,function(a){e.methods[a]=function(){var c=[a];angular.forEach(arguments,function(a){c.push(a)});return b.autocomplete.apply(b,c)}}),e.methods.filter=m,e.methods.clean=j,d.$watch(function(){return e.options},function(a){b.autocomplete("option",p(a))}),b.on("focus",k),b.autocomplete(angular.extend({},e.options,q)))}}}]);

angular.module("ui.bootstrap",["ui.bootstrap.tpls","ui.bootstrap.pagination"]);angular.module("ui.bootstrap.tpls",["template/pagination/pagination.html"]);
angular.module("ui.bootstrap.pagination",[]).constant("paginationConfig",{boundaryLinks:!1,directionLinks:!0,firstText:"First",previousText:"Previous",nextText:"Next",lastText:"Last"}).directive("pagination",["paginationConfig",function(b){return{restrict:"EA",scope:{numPages:"=",currentPage:"=",maxSize:"=",onSelectPage:"&"},templateUrl:"template/pagination/pagination.html",replace:!0,link:function(a,l,c){function e(a,c,d,b){return{number:a,text:c,active:d,disabled:b}}var f=angular.isDefined(c.boundaryLinks)?
a.$eval(c.boundaryLinks):b.boundaryLinks,g=angular.isDefined(c.directionLinks)?a.$eval(c.directionLinks):b.directionLinks,h=angular.isDefined(c.firstText)?c.firstText:b.firstText,i=angular.isDefined(c.previousText)?c.previousText:b.previousText,j=angular.isDefined(c.nextText)?c.nextText:b.nextText,k=angular.isDefined(c.lastText)?c.lastText:b.lastText;a.$watch("numPages + currentPage + maxSize",function(){a.pages=[];var c=a.maxSize&&a.maxSize<a.numPages?a.maxSize:a.numPages,b=a.currentPage-Math.floor(c/
2);1>b&&(b=1);b+c-1>a.numPages&&(b-=b+c-1-a.numPages);for(var d=b,c=b+c;d<c;d++)b=e(d,d,a.isActive(d),!1),a.pages.push(b);g&&(d=e(a.currentPage-1,i,!1,a.noPrevious()),a.pages.unshift(d),d=e(a.currentPage+1,j,!1,a.noNext()),a.pages.push(d));f&&(d=e(1,h,!1,a.noPrevious()),a.pages.unshift(d),d=e(a.numPages,k,!1,a.noNext()),a.pages.push(d));a.currentPage>a.numPages&&a.selectPage(a.numPages)});a.noPrevious=function(){return 1===a.currentPage};a.noNext=function(){return a.currentPage===a.numPages};a.isActive=
function(b){return a.currentPage===b};a.selectPage=function(b){!a.isActive(b)&&(0<b&&b<=a.numPages)&&(a.currentPage=b,a.onSelectPage({page:b}))}}}}]);angular.module("template/pagination/pagination.html",[]).run(["$templateCache",function(b){b.put("template/pagination/pagination.html",'<div class="pagination"><ul>  <li ng-repeat="page in pages" ng-class="{active: page.active, disabled: page.disabled}"><a ng-click="selectPage(page.number)">{{page.text}}</a></li>  </ul></div>')}]);
...
(function(b){var c=b("<div/>",{id:("spotlight-"+window.Math.random()).replace(".","")}),d=b("<div/>");b.fn.spotlight=function(a){if(!this.length)return this;a=b.extend({},b.fn.spotlight.defaults,a);c.addClass("animation-fade-in-short").css({position:"absolute",top:0,left:0,width:b(document).width(),height:b(document).height(),"z-index":a.zIndex,"pointer-events":a.clickThrough?"none":"auto"});d.css({position:"absolute",transition:"all "+a.transitionDuration+"s ease-in-out","box-shadow":"0px 0px 0px 9001px rgba(0, 0, 0, "+
a.opacity+"), inset 0px 0px "+a.insetShadowBlurRadius+"px "+a.insetShadowSpreadRadius+"px rgba(0, 0, 0, "+a.opacity+")","z-index":a.zIndex,"pointer-events":a.clickThrough?"none":"auto"});b("#"+c.attr("id")).length||b("body").append(c.append(d));d.css({top:this.offset().top-a.padding,left:this.offset().left-a.padding,width:this.outerWidth()+2*a.padding,height:this.outerHeight()+2*a.padding});return this};b.fn.spotlight.clear=function(){var a=new b.Deferred;c.one("webkitAnimationEnd msAnimationEnd animationend",
function(){a.resolve()});c.addClass("animation-fade-out-short");return a.promise().then(function(){c.removeClass("animation-fade-in-short animation-fade-out-short");c.remove()})};b.fn.spotlight.defaults={transitionDuration:0.5,opacity:0.5,backgroundColor:"black",zIndex:1E4,insetShadowSpreadRadius:7,insetShadowBlurRadius:8,insetShadowColor:"black",clickThrough:!1,padding:10}})(jQuery);
...
d?a=i(w(a),!0):(a=B(w(a)),a=a.replace(/\n$/,""),a=q(a));d=b;return"<li>"+a+"</li>\n"}),a=a.replace(/~0/g,"");s--;return a}function C(a){a=a.replace(/&/g,"&amp;");a=a.replace(/</g,"&lt;");a=a.replace(/>/g,"&gt;");return a=m(a,"*_{}[]\\",!1)}function E(a){a=a.replace(/&(?!#?[xX]?(?:[0-9a-fA-F]+|\w+);)/g,"&amp;");return a=a.replace(/<(?![a-z\/?!]|~D)/gi,"&lt;")}function w(a){a=a.replace(/^(\t|[ ]{1,4})/gm,"~0");return a=a.replace(/~0/g,"")}function D(a){if(!/\t/.test(a))return a;var c=["    ","   ",
...
k.preConversion(a);a=a.replace(/~/g,"~T");a=a.replace(/\$/g,"~D");a=a.replace(/\r\n/g,"\n");a=a.replace(/\r/g,"\n");a=D("\n\n"+a+"\n\n");a=a.replace(/^[ \t]+$/mg,"");a=e(a);a=a.replace(/^[ ]{0,3}\[(.+)\]:[ \t]*\n?[ \t]*<?(\S+?)>?(?=\s|$)[ \t]*\n?[ \t]*((\n*)["(](.+?)[")][ \t]*)?(?:\n+)/gm,function(a,b,d,e,h,i){b=b.toLowerCase();l.set(b,E(d));if(h)return e;i&&n.set(b,i.replace(/"/g,"&quot;"));return""});a=i(a);a=a.replace(/~E(\d+)E/g,function(a,b){var d=parseInt(b);return String.fromCharCode(d)});
a=a.replace(/~D/g,"$$");a=a.replace(/~T/g,"~");a=k.postConversion(a);r=n=l=null;return a};var t={ol:"\\d+[.]",ul:"[*+-]"},G=/(?:["'*()[\]:]|~D)/g}})();
// Sanitizer
(function(){function l(a){return a.replace(/<[^>]*>?/gi,m)}function m(a){return a.match(n)||a.match(p)||a.match(q)||a.match(r)?a:""}function s(a){if(""==a)return"";var d=/<\/?\w+[^>]*(\s|$|>)/g,f=a.toLowerCase().match(d),e=(f||[]).length;if(0==e)return a;for(var h,c,g,i=[],j=[],k=!1,b=0;b<e;b++)if(h=f[b].replace(/<\/?(\w+).*/,"$1"),!(i[b]||-1<"<p><img><br><li><hr>".search("<"+h+">"))){c=f[b];g=-1;if(!/^<\//.test(c))for(c=b+1;c<e;c++)if(!i[c]&&f[c]=="</"+h+">"){g=c;break}-1==g?k=j[b]=!0:i[g]=!0}if(!k)return a;
b=0;return a=a.replace(d,function(a){a=j[b]?"":a;b++;return a})}var d,e;"object"===typeof exports&&"function"===typeof require?(d=exports,e=require("./Markdown.Converter").Converter):(d=window.Markdown,e=d.Converter);d.getSanitizingConverter=function(){var a=new e;a.hooks.chain("postConversion",l);a.hooks.chain("postConversion",s);return a};var n=/^(<\/?(b|blockquote|code|del|dd|dl|dt|em|h1|h2|h3|i|kbd|li|ol|p|pre|s|sup|sub|strong|strike|ul)>|<(br|hr)\s?\/?>)$/i,q=/^(<a\s(\s?target\s?=\s?"_blank")?\s?href="((https?|ftp):\/\/|\/)[-A-Za-z0-9+&@#\/%?=~_|!:,.;\(\)]+"(\starget\s?=\s?"_blank")?\s?(\stitle="[^"<>]+")?\s?(\sdownload="[^"<>]+")?\s?>|<\/a>)$/i,
p=/^(<\/?span\s?(class="[-A-Za-z]*")?\s?>)$/i,r=/^(<img\ssrc="(https?:\/\/|\/)[-A-Za-z0-9+&@#\/%?=~_|!:,.;\(\)]+"(\swidth="\d{1,3}")?(\sheight="\d{1,3}")?(\salt="[^"<>]*")?(\stitle="[^"<>]*")?\s?\/?>)$/i})();
