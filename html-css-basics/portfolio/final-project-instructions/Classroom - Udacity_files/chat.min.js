...
a}):null};a.isInBookmarks=function(a){return e.hasBookmark(a.id)};a.leaveGroupAndRemoveBookmark=function(a){e.leaveRoom(a)}}}}]);angular.module("utility").directive("chatPanel",["cacheVersion","chatDataStore","currentUser","fetchDiscourseLink","$window","notifications",function(c,e,a,i,g,j){return{templateUrl:"/templates/chat/chat_panel.html?"+c,link:function(c){c.purposes=["topic"];c.selectedCourse=e.getSelectedCourse();c.enrolledNanodegreeKeys=[];a.fetchEnrolledAndGraduatedNanodegreeKeys().then(function(a){c.enrolledNanodegreeKeys=_.filter(a,function(a){return"nd801"!==a})});c.userName=a.get("nickname")||a.get("first_name");
c.getGroupsByPurpose=e.getRoomsByPurpose;i().then(function(a){c.forumLink=a});c.notificationsMuted=!0;c.updateSelectedCourse=function(){e.setSelectedCourse(c.selectedCourse.courseKey)};c.isRoomInSelectedCourse=function(a){return a.course===c.selectedCourse.courseKey};c.toggleNotifications=function(){j.toggleMuteSettings("chat");c.notificationsMuted=!c.notificationsMuted};c.openForum=function(){c.forumLink&&g.open(c.forumLink)};c.numBookmarks=function(){return _.size(_.filter(e.getBookmarks(),function(a,
b){return e.getRoomInfo(b).course===c.selectedCourse.courseKey}))};c.getMaxNumberOfChatBoxes=function(){return 992<$(g).width()?2:1};var d=function(){_.size(e.getOpenChannels())>=c.getMaxNumberOfChatBoxes()&&e.pruneOpenChannels(c.getMaxNumberOfChatBoxes())},b=function(){d()};$(g).resize(b);c.$on("$destroy",function(){$(g).off("resize",b)});c.joinGroup=function(a){e.isInOpenChannels(a)||(e.hasBookmark(a.id)||e.saveBookmark({id:a.id,name:a.name}),e.activateChannel(a.id),d());e.unminimizeChannel(a)};
